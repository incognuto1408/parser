            ...
            foreach ($tr->find('td') as $key1 => $td) {
                ...
                if($td->attr("align") == "right"){
                    ...
                    if(!empty($html_info["data"])) {
                        ...
                        if(!$data) {
                            $query = "INSERT INTO `all_list_domain` (`full_info`,`organization_name`,`domain_name`,`datetime_created`,`user_name`,`phone_number`, `email`, `city`, `street_address`, `registar_created`, `datetime_add`, `datetime_created1`, `datetime_created2`, `datetime_created3`) VALUES (:full_info, :organization_name, :domain_name, :datetime_created, :user_name, :phone_number, :email, :city, :street_address, :registar_created, :datetime_add, :datetime_created1, :datetime_created2, :datetime_created3)";
                            $stmt = $db->prepare($query);
                            $exp = explode("(", $tmp[$key][':datetime_created']);
                            $tmp[$key][':datetime_created1'] = date("Y-m-d H:i:s", strtotime($exp[0]));
                            $tmp[$key][':datetime_created2'] = date("Y-m-d H:i:s", strtotime($exp[0]));
                            $tmp[$key][':datetime_created3'] = "dsdfs";
                            echo $tmp[$key][':datetime_created3'];
                            echo " asasas ".$tmp[$key][':datetime_created'];
