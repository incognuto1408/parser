...
require_once 'systems/fn/query.php';*/
                ...
                if($td->attr("align") == "right"){
                    ...
                    if(!empty($html_info["data"])) {
                        ...
                        if(substr($tmp[$key][':phone_number'], 0, 2) === "77" || substr($tmp[$key][':phone_number'], 0, 2) === "87")
                            if (substr($tmp[$key][':phone_number'], 2, 2) !== "27")
                                $error_list['s_num'] = 0;
                        ...
                        if($error_list['domain']){
                            $sql1 = "SELECT * FROM black_list_domain WHERE domain = '{$tmp[$key]['domain_name']}' LIMIT 1";
                        ...
                        if($error_list['s_num'] || $error_list['num_str'] ){
                            $sql1 = "SELECT * FROM black_list_domain WHERE number = '{$tmp[$key][':phone_number']}' LIMIT 1";
                            $result1 = $db->query($sql1);
                            $data1 = $result1->fetch(PDO::FETCH_ASSOC);
                            if(!$data1){
                                $query2 = "INSERT INTO `black_list_domain`(`number`)VALUES(:number)";
                                $stmt2 = $db->prepare($query2);
                                $stmt2->execute(array(":number" => trim($tmp[$key][':phone_number'])));
                            }
                        }
...
/*                            $array_num = ["-", "+", ".", " "];
                            $tmp[$key][':phone_number'] = str_replace($array_num, "", $tmp[$key][':phone_number']);
                            $start_s_num = 1;
                            if((substr("$tmp[$key][':phone_number']", 0, 2) === "77" || substr("$tmp[$key][':phone_number']", 0, 2) === "87") && substr("$tmp[$key][':phone_number']", 2, 4) !== "27")
                                $start_s_num = 0;
                            if(strlen($tmp[$key][':domain_name']) > 21)
                                $start_s_num = 1;
                            if($start_s_num){
                                ...
                                if(!$sql1){
                                    $query2 = "INSERT INTO `black_list_domain`(`number`)VALUES(:number)";
                                    $stmt2 = $db->prepare($query2);
                                    $stmt2->execute(array(":number" => $tmp[$key][':phone_number']));
                                }
                            }*/
