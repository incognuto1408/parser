            ...
            foreach ($tr->find('td') as $key1 => $td) {
                ...
                if($td->attr("align") == "right"){
                    ...
                    if(!empty($html_info["data"])) {
                        ...
                        $array_num = ["-", "+", ".", " ", "", "(", ")"];
                        $tmp[$key][':phone_number'] = str_replace($array_num, "", $tmp[$key][':phone_number']);
                        $error_list = [
                            "domain" => 0,
                            "s_num" => 1,
                            "num_str" => 0
                        ];
                        if(substr($tmp[$key][':phone_number'], 0, 2) === "77" || substr($tmp[$key][':phone_number'], 0, 2) === "87") {
                            if (substr($tmp[$key][':phone_number'], 2, 2) !== "27" && substr($tmp[$key][':phone_number'], 2, 2) !== "17") {
                                $error_list['s_num'] = 0;
                                $tmp[$key][':phone_number'] = "7" . substr($tmp[$key][':phone_number'], 1);
                            }
                        }
                        if(strlen($tmp[$key][':phone_number']) < 11)
                            $error_list['num_str'] = 1;

                        if(strlen($tmp[$key][':domain_name']) > 21)
                            $error_list['domain'] = 1;



                        if($error_list['domain']){

                            $data1 = db_query("SELECT * FROM black_list_domain WHERE domain = '{$tmp[$key]['domain_name']}' LIMIT 1");
                            if(count($data1) == 0){
                                $query2 = "INSERT INTO `black_list_domain`(`domain`)VALUES(:domain)";
                                $stmt2 = $db->prepare($query2);
                                $stmt2->execute(array(":domain" => trim($tmp[$key]['domain_name'])));
                            }
                        }
                        if($error_list['s_num'] || $error_list['num_str'] ){
                            $data1 = db_query("SELECT * FROM black_list_domain WHERE number = '{$tmp[$key][':phone_number']}' LIMIT 1");
                            if(count($data1) == 0){
                                db_insert_update("INSERT INTO black_list_domain (number) VALUES ('".trim($tmp[$key][':phone_number'])."')");
                        ...
                        $data = db_query("SELECT * FROM all_list_domain WHERE domain_name = '{$tmp[$key][':domain_name']}' LIMIT 1");
                        if(count($data) == 0){
                            $exp = explode("(", $tmp[$key][':datetime_created_unique']);
                            $tmp[$key][':datetime_created'] = date("Y-m-d H:i:s", strtotime($exp[0]));


                            db_insert_update("INSERT INTO all_list_domain (full_info,organization_name,domain_name,datetime_created,datetime_created_unique,user_name,phone_number, email, city, street_address, registar_created, datetime_add, send_message) VALUES ('".$tmp[$key][':full_info']."', '".$tmp[$key][':organization_name']."', '".$tmp[$key][':domain_name']."', '".$tmp[$key][':datetime_created']."', '".$tmp[$key][':datetime_created_unique']."', '".$tmp[$key][':user_name']."', '".$tmp[$key][':phone_number']."', '".$tmp[$key][':email']."', '".$tmp[$key][':city']."', '".$tmp[$key][':street_address']."', '".$tmp[$key][':registar_created']."', '".$tmp[$key][':datetime_add']."', '".$settings['settings']['mailing']."')");
                        }
                    }
