                    ...
                    if(!empty($html_info["data"])) {
                        ...
                        if(!$data) {
                            ...
                            if(strlen($tmp[$key][':domain_name']) > 21)
                                $str_domain = 0;
                            if($start_s_num && $str_domain){
                                $sql_black = db_query("SELECT * FROM black_list_domain WHERE number = '{$tmp[$key][':phone_number']}' LIMIT 1");
                                if(!$sql_black){
                                    db_insert_update("INSERT INTO black_list_domain(number)VALUES('".$tmp[$key][':phone_number']."')");
                                }
                            }
